...
class AuthManager:
    ...
    def login(self, username, password) -> Dict[str, Any]:
        ...
        if not LINSHARE_USER_URL:
            raise ValueError("LINSHARE_USER_URL not configured")
            
        url = f"{LINSHARE_USER_URL}/authentication/jwt"
    ...
    def _fetch_user_info(self):
        ...
        if not self.token or not LINSHARE_USER_URL:
            return
        
        try:
            # Use the /authentication/authorized endpoint to get user info
            url = f"{LINSHARE_USER_URL}/authentication/authorized"
            response = requests.get(
                url,
                headers={
